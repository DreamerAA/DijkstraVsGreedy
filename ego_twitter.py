...
data = pd.read_csv(
    "../../dataset/ego_twitter/twitter_combined.csv", delimiter=' ')
id1 = data["0"].to_numpy()
id2 = data["1"].to_numpy()
...
# a_u = np.array([10**i for i in range(5, 13)])
# a_p = np.arange(0.01, 0.42, step=0.04)
# # a_u = [1e4, 1e5]
# # a_p = [0.1, 0.2]
...
# df.to_netcdf(
#     f"facebook_small_new_try.nc")

nx.draw_shell(igraph)
# show_graph(igraph)
# nx.draw(igraph)
plt.show()
