...
def fillDict(path="../Results/directed_scale_free_graphs/"):
...
def generate_graphs(path="../Results/directed_scale_free_graphs/"):
    ...
    def create(n, a, b, g):
        ...
        graph = GraphCreator.generateDirectedScaleFreeGraph(n, a, b, g)
    ...
    for n in range(600, 2, -1):
        for a in np.arange(0.1, 0.9, 0.1):
            for b in np.arange(0.1, 0.9, 0.1):
                Parallel(n_jobs=11)(delayed(create)(n, a, b, g)
...
def simulation_zch():
    ...
    fname = f"../Results/DirectedScaleFreeGraphResults/c_from_{cc[0]}_to_{cc[-1]}_d_from_{cd[0]}_to_{cd[-1]}_u={u}_p={p}.nc"
...
def simulation_zup():
    ...
    fname = f"../Results/DirectedScaleFreeGraphResults/u_from_{a_u[0]}_to_{a_u[-1]}_p_from_{a_p[0]}_to_{a_p[-1]}_c={rc}_D={rd}.nc"
...
def main():
    ...
    # simulation_zup()
    fname = "u_from_10.0_to_1000000000.0_p_from_0.0_to_0.39_c=8_D=8.nc"
    Visualizer.showRegularResult(
        f"../Results/DirectedScaleFreeGraphResults/{fname}", field="ln10_z", xticks=[0, 0.1, 0.2, 0.3])
    Visualizer.add_critical_u(4, 0.39)

    # simulation_zch()
    # fname = "c_from_2_to_8_d_from_2_to_8_u=10000.0_p=0.01.nc"
    # Visualizer.showRegularResult(
    #     f"../Results/DirectedScaleFreeGraphResults/{fname}", field="ln10_z")
